<?php
class AdminPixCoreFormRenderer implements PixCoreFormRendererInterface
{
    /**
     * @var PixCoreFormManagerInterface
     */
    private $manager;
    ...
    function render(PixCoreFormField $form)
    {
        switch ($form->getType()) {
            case 'main':
                echo "<div class=\"wrap\">";
...
//		        array_map(array($this, 'render'), $form->getMainField());
    ...
    function setManager(PixCoreFormManagerInterface $manager)
    {
        $this->manager = $manager;
    }

    function getName(PixCoreFormField $form)
    {
        if ($this->prefix != "") {
    ...
    function getInput(PixCoreFormField $form, $type = 'text')
    {
	...
	function getCheckbox (PixCoreFormField $form)
	{
		$value = $this->manager->get($form->getName(), @$form->getAttributes()['default']);
		$checked = '';
		if ( $value === "on" ) {
